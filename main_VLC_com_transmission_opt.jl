    ...
    for ps_index in eachindex(ps_real)
        ...
        for loop_index in 1:simulation_loop
            ...
            for n in 1:user_num
                ...
                user_SINR = (sum(h_user[n,:])^2 * ps_real[ps_index] * β[n]
                / (sum(h_user[n,:])^2 * ps_real[ps_index] * sum(β[n+1:user_num]) + n0))
                # println(user_SINR)
                capacity_user += 0.5 * log2(1 + user_SINR)

                user_est_SINR = (sum(h_user_est[n,:])^2 * ps_real[ps_index] * β[n]
                / (sum(h_user_est[n,:])^2 * ps_real[ps_index] * sum(β[n+1:user_num]) + n0))
