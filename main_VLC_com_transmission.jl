    ...
    for ps_index in eachindex(ps_real)
        ...
        for loop_index in 1:simulation_loop
            ...
            for n in 1:user_num
                ...
                user_SINR = (sum(h_user[n,:])^2 * ps_real[ps_index] * β_sum 
                / (sum(h_user[n,:])^2 * ps_real[ps_index] * ((1 - β)^(n-1) - β_sum) + n0))
                # println(user_SINR)
                capacity_user += 0.5 * log2(1 + user_SINR)

                # eve_SINR = (sum(h_eve)^2 * ps[ps_index] * β_sum 
                # / (sum(h_eve)^2 * ps[ps_index] * ((1 - β)^(n-1) - β_sum) + n0))
        ...
        # capacity_eve_simu[ps_index] = sum_eve / simulation_loop
        # secrecy_simu[ps_index] = sec / simulation_loop
        print("\n")
    end

    # output file
    path = string("results/case1_trans/Loop_num=", Int(simulation_loop), 
