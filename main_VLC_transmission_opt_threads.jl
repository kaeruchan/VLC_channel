...
function main()
    ...
    # simulation_loop_vec = zeros(simulation_loop)
    ...
    for ps_index in eachindex(ps)
        ...
        sum_user = 0
        sum_user_est = 0
        # user = zeros(simulation_loop)
        # user_est = zeros(simulation_loop)
        @threads for loop_index in ProgressBar(1:simulation_loop)
            
            # Threads.atomic_add!(loop_simu_ato,loop_index)
            ...
            # simulation_loop_vec[loop_index] = loop_index
            # now_loopnum = count!(!iszero,simulation_loop_vec)
            # print("\r",
            # "Ps = ", ps[ps_index],
            # ", Trans_capacity_fact = ", sum(user) / now_loopnum,
            # ", Trans_capacity_est = ", sum(user_est) / now_loopnum,
            # ", Simulation Period = ", now_loopnum)
        end
        ...
        # capacity_eve_simu[ps_index] = sum_eve_simu / simulation_loop
