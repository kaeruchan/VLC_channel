...
import .ProjectVLC.Parameters: ψ_c, ψ_05, I_DC, Nb, u_r, A_PD, β, η, height, device_height, N0, height_user_body, shoulder_width, x_eve, y_eve, led_com, user_coop, led_com_to_led_ratio
...
function main()
            ...
            for loop_index in 1:simulation_loop
                ...
                for n in 1:user_num
                        ...
                        * β_sum_per_led[n,led_indice_user] 
                        / (
                            ...
                            * sum(β_sum_per_led[n+1:user_num,led_indice_user])
                            + (sum(h_user_rec[n,:]) - h_user_rec[n,led_indice_user])^2 * ps + n0
                            )
                        )
                    ...
                    if h_eve[led_indice_user] != 0.0
                        eve_SINR = (h_eve[led_indice_user]^2 
                        * ps 
                        * β_sum_per_led[n,led_indice_user]
                        / (
                            h_eve[led_indice_user]^2 
                            * ps 
                            * sum(β_sum_per_led[n+1:user_num,led_indice_user])
    ...
    file_eve = string("VLC_eve_com.txt")
    file_sec = string("VLC_sec_com.txt")
