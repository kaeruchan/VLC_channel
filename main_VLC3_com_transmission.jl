...
function main()
        ...
        for loop_index in 1:simulation_loop
            ...
            for s in eachindex(user_group)
                ...
                for b in 1:length(user_group[s])
                    ...
                    user_SINR = (sum(h_user[user_group[s][b],s_group[s]])^2 * ps_real[ps_index] * β_sum[b] 
                    / (sum(h_user[user_group[s][b],s_group[s]])^2 * ps_real[ps_index] * ((1-β)^(b-1) - β_sum[b]) + n0))
                    # println(β_sum[b])
    ...
    # output file
    path = string("results/case3_trans/Loop_num=", Int(simulation_loop), 
