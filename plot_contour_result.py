...
def main(argv):
    ...
    # axes.set_zticks(np.arange(0,20,5),size=14)
    
    
    surf = axes.contourf(x,y,z,levels=levels,cmap='hot')
    ...
    axes.set_xlabel('$L$~(m)', fontdict={'size': 16})
    axes.set_ylabel('$W$~(m)', fontdict={'size': 16})
    # if res == 'sec':
    ...
    # axes.set_zlabel(zlabel, fontdict={'size': 12})
    fig.colorbar(surf)
    
    dirs = 'output/figure/case' + str(case) + '/Loop_num=' + str(loop) + '/Ps=' + str(ps) + '/user_type' + str(user) + '/contour'
